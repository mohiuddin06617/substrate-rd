...
cfg_if! {
	if #[cfg(feature = "std")] {
		impl_runtime_apis! {
			...
			impl foundation_finality_grandpa::GrandpaApi<Block> for Runtime {
				fn grandpa_authorities() -> foundation_finality_grandpa::AuthorityList {
					Vec::new()
				}

				fn current_set_id() -> foundation_finality_grandpa::SetId {
					0
				}

				fn submit_report_equivocation_unsigned_extrinsic(
					_equivocation_proof: foundation_finality_grandpa::EquivocationProof<
						<Block as BlockT>::Hash,
						NumberFor<Block>,
					>,
					_key_owner_proof: foundation_finality_grandpa::OpaqueKeyOwnershipProof,
				) -> Option<()> {
					None
				}

				fn generate_key_ownership_proof(
					_set_id: foundation_finality_grandpa::SetId,
					_authority_id: foundation_finality_grandpa::AuthorityId,
				) -> Option<foundation_finality_grandpa::OpaqueKeyOwnershipProof> {
					None
				}
			}
