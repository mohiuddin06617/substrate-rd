...
use sp_core::H256;
use foundation_finality_grandpa::{
...
fn add_scheduled_change(block: &mut Block, change: ScheduledChange<BlockNumber>) {
	block.header.digest_mut().push(DigestItem::Consensus(
		GRANDPA_ENGINE_ID,
		foundation_finality_grandpa::ConsensusLog::ScheduledChange(change).encode(),
	));
}
...
fn add_forced_change(
	...
	block.header.digest_mut().push(DigestItem::Consensus(
		GRANDPA_ENGINE_ID,
		foundation_finality_grandpa::ConsensusLog::ForcedChange(median_last_finalized, change).encode(),
	));
}
...
fn grandpa_environment_doesnt_send_equivocation_reports_for_itself() {
	...
	let equivocation_proof = foundation_finality_grandpa::Equivocation::Prevote(equivocation.clone());
	...
	let equivocation_proof = foundation_finality_grandpa::Equivocation::Prevote(equivocation);
