...
fn report_equivocation_current_set_works() {
	...
	new_test_ext_raw_authorities(authorities).execute_with(|| {
		...
		let key_owner_proof =
			Historical::prove((foundation_finality_grandpa::KEY_TYPE, &equivocation_key)).unwrap();
...
fn report_equivocation_invalid_key_owner_proof() {
	...
	new_test_ext_raw_authorities(authorities).execute_with(|| {
		...
		let invalid_key_owner_proof =
			Historical::prove((foundation_finality_grandpa::KEY_TYPE, &invalid_owner_key)).unwrap();
